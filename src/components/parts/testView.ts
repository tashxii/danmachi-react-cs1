...
import { strValOpt, RW, useCsInputTextItem, useCsSelectBoxItem, useCsCheckBoxItem, numValOpt, useCsInputNumberItem, useCsRadioBoxItem, useCsTextAreaItem, selectOpt, selectOptStr, useCsMultiCheckBoxItem, useCsPasswordItem } from "../../framework/cs/CsHooks";
...
export function useTestView(): TestView {
    const testView: TestView = {
        ...
        contactWay: useCsRadioBoxItem("連絡方法", useState(""), numValOpt(true), selectOptStr(["メール", "電話", "訪問"])),
        age: useCsInputNumberItem("年齢", useState(20), numValOpt(true, 18, 70)),
        memo: useCsTextAreaItem("メモ", useState(""), strValOpt(true, 1, 4000)), snsWay: useCsMultiCheckBoxItem("SNS連絡手段",
            useState(["SMS", "Twitter"]), selectOptStr(["SMS", "Line", "Twitter", "Facebook"])),
